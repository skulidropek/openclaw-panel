...
const bundleIncludeAttributesFromForm = (
  form: URLSearchParams,
  mode: BotBundleMode
): Effect.Effect<ReadonlyArray<BotBundleIncludeAttribute>, Error> => {
  const values = form.getAll("include").map((value) => value.trim()).filter((value) => value.length > 0)
  const includeAttributes = parseBotBundleIncludeAttributes(values)
  if (includeAttributes === null) {
    return Effect.fail(new Error("Unknown bundle include attribute."))
  }
  if (mode !== "private" && includeAttributes.length > 0) {
    return Effect.fail(new Error("Bundle include attributes are only available for private exports."))
  }
  return Effect.succeed(includeAttributes)
}
...
const handleExportBundleRequest = (
  runtime: PanelRuntime,
  botId: string,
  ...
  pipe(
    ...
    Effect.flatMap((form) => {
      ...
      return mode === null
        ...
        : pipe(
          bundleIncludeAttributesFromForm(form, mode),
          Effect.flatMap((includeAttributes) =>
            exportBotBundle(runtime, botId, mode, includeAttributes, requestOrigin(runtime, request))
          )
        )
    }),
...
const handlePostRequest = (
  ...
  if (bundleBotId !== null) {
    return handleExportBundleRequest(runtime, bundleBotId, request, response)
  }
