...
import { findBot, findOnboardingSession, type OnboardingSession, type PanelRuntime } from "../backend/panel-runtime.js"
...
const finalizeAndCloseSocket = (session: OnboardingSession, socket: Duplex): void => {
  ...
  pipe(
    ...
    Effect.matchEffect({
      onFailure: (error) =>
        Effect.sync(() => {
          writeSocket(socket, encodeTextFrame(`\n[daemon finalize error] ${errorMessage(error)}\n`))
          endSocket(socket)
        }),
      onSuccess: () =>
        Effect.sync(() => {
  ...
  return pipe(
    ...
    Effect.flatMap((session) =>
      startOnboardingProcess(session.containerId, {
        ...
        onEnd: () => {
          if (!clientClosed) {
            finalizeAndCloseSocket(session, socket)
          }
        },
        onError: (error) => {
...
const proxyBotAdminSocket = (
  ...
  pipe(
    ...
    Effect.matchEffect({
      onFailure: (error) =>
        Effect.sync(() => {
          rejectUpgrade(socket, `OpenClaw admin websocket failed: ${errorMessage(error)}`)
        }),
...
export const handlePanelUpgrade = (
  ...
  pipe(
    ...
    Effect.matchEffect({
      onFailure: (error) =>
        Effect.sync(() => {
          socket.write(encodeTextFrame(`\n[onboarding error] ${errorMessage(error)}\n`))
          socket.end()
        }),
      onSuccess: () => Effect.void
    }),
